...
class AuraCenter {
        ...
        const card = document.createElement("div");
        card.className =
            "hover-card classes_cards w-[50vw] lg:w-[30vw] h-[30vh] lg:h-[50vh] lg:hover:z-[3] cursor-pointer group flex-shrink-0";
        ...
        card.innerHTML = `
                ...
                <picture id="trainer-card-img-${index}" class="card__front">
                        ...
                        <div id="hover-card-content-${index}" class="rounded-xl card__back h-full w-full absolute top-0 left-0 bg-[#c7b6a8] lg:translate-x-0 transition-transform transition-opacity duration-[600ms] ease-out shadow-[0_20px_40px_rgba(0,0,0,0.3)] will-change-[transform] will-change-[opacity] cursor-pointer flex flex-col justify-center items-center lg:group-hover:translate-x-[100%] lg:group-hover:z-[1]">
                            ...
                            <ul class="space-y-2 text-lg text-black list-disc">
                            ${listItems
                            .map(
                                (item) => `
                                <li class="text-lg">
...
function slideRight(containerSelector, cardSelector) {
    const container = document.querySelector(containerSelector);
    const card = document.querySelector(cardSelector);
    const btnLeft = container.parentElement.parentElement.querySelector(".slide-left");
    const btnRight = container.parentElement.parentElement.querySelector(".slide-right");

    if (container && card) {
        const maxScroll = container.scrollWidth - container.clientWidth;
        const newScroll = Math.min(container.scrollLeft + card.offsetWidth, maxScroll);
        gsap.to(container, { scrollLeft: newScroll, duration: 0.5, ease: "power2.out" });
        updateButtons(btnLeft, btnRight, container.scrollLeft, maxScroll);
    }
}

function slideLeft(containerSelector, cardSelector) {
    const container = document.querySelector(containerSelector);
    const card = document.querySelector(cardSelector);
    const btnLeft = container.parentElement.parentElement.querySelector(".slide-left");
    const btnRight = container.parentElement.parentElement.querySelector(".slide-right");
    ...
    if (container && card) {
        ...
        const newScroll = Math.max(container.scrollLeft - card.offsetWidth, 0);

        // animate scrollLeft using GSAP
        gsap.to(container, {
            scrollLeft: newScroll,
            duration: 0.5,
            ease: "power2.out"
        });
        updateButtons(btnLeft, btnRight, container.scrollLeft, 0);
    }
}

function updateButtons(btnLeft, btnRight, scrollLeft, maxScroll) {
  btnLeft.classList.toggle("opacity-0", scrollLeft <= 0);
  btnRight.classList.toggle("opacity-0", scrollLeft >= maxScroll);
}
